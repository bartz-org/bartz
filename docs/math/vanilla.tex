...
($\vyt_i = \vy_i$), the leaf precision $\mLamm$ is fixed rather than drawn
from \eqref{eq:mv-Lamm-prior} (so the update of \S\ref{sec:mv-Lamm} is
skipped), the split-variable probabilities $\vs$ are fixed rather than drawn
from \eqref{eq:mv-s-prior} (so $\theta$ and its hyperprior play no role),
and the dictionary between the two chapters is:
\begin{center}
	\begin{tabular}{lll}
		\toprule
