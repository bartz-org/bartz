...
coincide, and give the univariate update of \S\ref{sec:sigma-update} with
$n$ replaced by the number of observed outcomes.
...
Bartz implements this chapter in \texttt{src/bartz/mcmcstep}. The form of
$\mLam$ is selected by \texttt{mcmcstep.init} from its arguments: the dense
form \eqref{eq:mv-iw-prior} (a \texttt{Wishart} prior) when
\texttt{outcome\_type} is continuous for all components and \texttt{missing}
is at most a per-datapoint mask, the diagonal form \eqref{eq:mv-gamma-prior}
(a \texttt{DiagWishart} prior) when some component is binary or
\texttt{missing} is a $k \times n$ mask; when all components are binary,
$\mLam$ is the identity with no prior (a \texttt{Wishart} with
\texttt{nu} $=$ \texttt{rate} $=$ \texttt{None}). The per-component scales
$1/\va_i$ are a $k \times n$ \texttt{error\_scale}, and a $k \times n$
\texttt{missing} mask sets the corresponding $a_{i,j} = 0$. The relevant
files are:
\begin{description}
	\item[\texttt{\_state.py}] The \texttt{State} and \texttt{Forest} classes
	      ...
	      \begin{itemize}
		      \item \texttt{State.error\_cov\_inv} is $\mLam$ with its
		            prior: for a \texttt{Wishart}, \texttt{nu} $= \nu$ and
		            \texttt{rate} $= \Psi$; for a \texttt{DiagWishart},
		            $\nu_j = \texttt{nu} - k + 1$ and $\psi_j =
			            \texttt{rate}_{jj}$ as in \eqref{eq:mv-gamma-match},
		            with $\texttt{rate}_{jj} = 0$ for the fixed binary
		            components;
		      \item \texttt{State.y} is $\vy_i$; \texttt{State.z} holds the
		            binary rows $z_{i,j}$, $j \in \setB$, of $\vyt_i$, at the
		            indices \texttt{State.binary\_indices}; the residuals
		            \texttt{State.resid} are $\vyt_i - f(\vx_i)$;
	...
	\item[\texttt{\_step.py}] The Gibbs sweep and the leaf and noise model.
	      \texttt{step} updates in order the trees, the latent outcomes, the
	      error precision, and the variable selection parameters:
	      \begin{itemize}
		      ...
		      \item \texttt{step\_z} draws from \eqref{eq:mv-z-fc};
