...
\begin{equation}
	\label{eq:subst}
	\begin{alignedat}{4}
		\setB &= \emptyset,  & \qquad k      &= 1,        & \qquad \va_i &\equiv 1,       & \qquad w &\equiv 1, \\
		\mLam &= 1/\sigsq,   & \qquad \mLamm &= 1/\smsq,  & \qquad \Psi  &= \nu\lambda .
	\end{alignedat}
\end{equation}
All $k \times k$ matrices become scalars, there are no latent outcomes
($\vyt_i = \vy_i$), and the dictionary between the two chapters is:
\begin{center}
	\begin{tabular}{lll}
		\toprule
...
Unchanged from \S\ref{sec:mv-tree-prior}: a node at depth $d$ is internal
with probability $p_{\mathrm{NT}}(d) = \alpha(1+d)^{-\beta}$ of
\eqref{eq:pnt}, truncated at $d_{\max}$, and given that it is internal its
splitting rule $(v, c)$ is uniform on the admissible variable/cutpoint pairs.
...
The joint density \eqref{eq:mv-joint} reads
\begin{equation}
	\label{eq:joint}
...
The graphical model is that of \S\ref{sec:mv-bn} without the $\va_i$ input
node:
\begin{center}
...
\begin{equation}
	...
	(1 - p_{d_\eta+1})^{2},
\end{equation}
with the same convention for non-growable children.
